...
class RendezvousChannel(object):
    """
    ...
    def empty(self):
        ...
        return not self._queue  # and not self._putters # putters will always be non-empty if queue is, right?
    ...
    def full(self):
        ...
        """
        return self.qsize >= len(self._getters)
    ...
    async def send(self, item):
        ...
        # slow path, but an item needs to be in the deque for get to fast path
        self._put(item)
        while self.full and not self._close.is_set():
            ...
            try:
                ...
                raise
            except Exception:  # todo what is this for, and is it still necessary
        ...
        if self.closed:
            raise ChannelClosed
        # self._wakeup_next(self._getters) # only waiting getters on fast path
        ...
        while self.empty and not self._close.is_set():
            ...
            try:
                ...
                if not self.empty and not getter.cancelled():
                    ...
                    self._wakeup_next(self._getters)
                raise
        return self.poll(fast)

    def poll(self, fast=True):
        """Remove and return an item from the channel.
        ...
        if self.empty and self._close.is_set():
            ...
            self._finished.set()
        if fast:
            # there will only be a waiting putter if we are on fast path
            self._wakeup_next(self._putters)
        return item
